...
module.exports = (app) => {
  ...
  passport.use('facebook', new FacebookStrategy({
    ...
    console.log('email is:', profile.emails[0].value); // this was returning undefined
    ...
    if (userResult.length == 0) {
      let user = { // when using Postgresql to access table, can't use capital letters for columns - using knex, you can use / create tables with uppercase column names
        ...
        // profilepicture:`https://graph.facebook.com/${profile.id}/picture?width=200&height=200` // using facebook graph api
        accesstoken: accessToken,
        // enableProof: true // if you delete the user from table, this will throw an error if you re-add user
      }
  ...
  passport.use('google', new GoogleStrategy({
    ...
    let newSize = "200",
            str = profile.photos[0].value,
         resize = str.split("?sz=50")[0]+"?sz="+newSize;

    console.log('user is:', profile);
    console.log('email is:', profile.emails[0].value);
    console.log('profile photo:', resize); //profile.photos[0].value.split("?sz=50")[0]+"?sz="+"200";
